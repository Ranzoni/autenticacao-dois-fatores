...
namespace AutenticacaoDoisFatores.Teste.Servico
{
    ...
    public class UsuarioServicoTeste
    {
        ...
        internal async Task DeveCadastrar()
        {
            ...
            var usuarioResposta = new UsuarioResposta(nome, email, DateTime.Now, false);
        ...
        internal async Task DeveAlterarNome()
        {
            ...
            var usuarioResposta = new UsuarioResposta(nome, usuarioCadastrado.Email, DateTime.Now, true);
            ...
            Assert.Equal(senha, usuarioCadastrado.Senha);
        ...
        internal async Task NaoDeveAlterarComNomeInvalido(string nomeInvalido)
        {
            ...
            var id = _faker.Random.Int(1);
            var chave = Guid.NewGuid();
            var usuarioAlterar = new UsuarioAlterar(nomeInvalido);
            var urlBase = _faker.Internet.Url();
        ...
        internal async Task NaoDeveAlterarQuandoNaoExiste()
        {
            ...
            var novaSenha = _faker.Random.AlphaNumeric(10);
            var usuarioAlterar = new UsuarioAlterar(novoNome, novaSenha);
        ...
        internal async Task NaoDeveEnviarEmailAlteracaoEmailComEmailInvalido(string emailInvalido)
        {
            ...
            var id = _faker.Random.Int(1);
            var chave = Guid.NewGuid();
            var usuarioAlterar = new UsuarioAlterarEmail(emailInvalido);
            var urlBase = _faker.Internet.Url();
        ...
        internal async Task DeveAlterarEmail()
        {
            ...
            var usuarioResposta = new UsuarioResposta(usuarioCadastrado.Nome, novoEmail, usuarioCadastrado.DataCadastro, true);
        ...
        [Fact]
        internal async Task DeveEnviarEmailAlteracaoSenha()
        {
            var servico = _mocker.CreateInstance<UsuarioServico>();
            var chave = Guid.NewGuid();
            var senha = _faker.Random.AlphaNumeric(10);
            ...
            Assert.True(retorno);
            _mocker.GetMock<IEmailServico>().Verify(d => d.EnviarEmailConfirmacaoAlteracaoSenha(usuarioCadastrado.Email, usuarioCadastrado.Nome, It.IsAny<string>()), Times.Once);
        }
        ...
        [InlineData("")]
        [InlineData("    ")]
        internal async Task NaoDeveEnviarEmailAlteracaoSenhaComValorInvalido(string senhaInvalida)
        {
            var servico = _mocker.CreateInstance<UsuarioServico>();
            var id = _faker.Random.Int(1);
            var chave = Guid.NewGuid();
            var usuarioAlterar = new UsuarioAlterarSenha(senhaInvalida, chave);
            var urlBase = _faker.Internet.Url();

            var retorno = await servico.EnviarEmailAlteracaoSenhaAsync(id, usuarioAlterar, urlBase);

            Assert.False(retorno);
            _mocker.GetMock<IUsuarioDominio>().Verify(d => d.BuscarAsync(It.IsAny<int>(), It.IsAny<Guid>()), Times.Never);
            _mocker.GetMock<IEmailServico>().Verify(d => d.EnviarEmailConfirmacaoAlteracaoSenha(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>()), Times.Never);
            _mocker.GetMock<INotificador>().Verify(n => n.AddMensagem(NotificacoesUsuario.SenhaInvalida), Times.Once);
        }

        [Fact]
        internal async Task NaoDeveEnviarEmailAlteracaoSenhaComEmailUsuarioInexistente()
        {
            var servico = _mocker.CreateInstance<UsuarioServico>();
            var id = _faker.Random.Int(1);
            var chave = Guid.NewGuid();
        ...
        [Fact]
        internal async Task DeveConfirmarAlteracaoSenha()
        {
            var servico = _mocker.CreateInstance<UsuarioServico>();
            var novaSenha = _faker.Random.AlphaNumeric(10);
            var chave = _faker.Random.Guid();
            ...
            _mocker.GetMock<IUsuarioDominio>().Setup(d => d.BuscarAtivoAsync(usuarioCadastrado.Id, chave)).ReturnsAsync(usuarioCadastrado);
            var usuarioResposta = new UsuarioResposta(usuarioCadastrado.Nome, novaSenha, usuarioCadastrado.DataCadastro, true);
            _mocker.GetMock<IMapper>().Setup(m => m.Map<UsuarioResposta>(usuarioCadastrado)).Returns(usuarioResposta);

            var retorno = await servico.AlterarSenhaAsync(token);
        ...
        [Fact]
        internal async Task NaoDeveAlterarSenhaComUsuarioInexistente()
        {
            var servico = _mocker.CreateInstance<UsuarioServico>();
            var id = _faker.Random.Int(1);
            var novaSenha = _faker.Random.AlphaNumeric(10);
            var chave = _faker.Random.Guid();
            var token = Token.GerarTokenAlterarSenhaUsuario(id, novaSenha, chave);
        ...
        [Fact]
        internal async Task DeveRetornarUsuario()
        {
            var servico = _mocker.CreateInstance<UsuarioServico>();
            var chave = Guid.NewGuid();
            var id = _faker.Random.Int(1);
            var usuario = new UsuarioConstrutor()
                .ComId(id)
                .CriarCompleto();
            _mocker.GetMock<IUsuarioDominio>().Setup(d => d.BuscarAsync(id, chave)).ReturnsAsync(usuario);
            var usuarioResposta = new UsuarioResposta(usuario.Nome, usuario.Email, usuario.DataCadastro, usuario.Ativo);
            _mocker.GetMock<IMapper>().Setup(m => m.Map<UsuarioResposta?>(usuario)).Returns(usuarioResposta);

            var retorno = await servico.BuscarAsync(id, chave);

            Assert.NotNull(retorno);
            Assert.Equal(usuarioResposta, retorno);
            _mocker.GetMock<IUsuarioDominio>().Verify(d => d.BuscarAsync(id, chave), Times.Once);
        }

        [Fact]
        internal async Task DeveRetornarNuloQuandoUsuarioNaoExiste()
        {
            var servico = _mocker.CreateInstance<UsuarioServico>();
            var chave = Guid.NewGuid();
            var id = _faker.Random.Int(1);
